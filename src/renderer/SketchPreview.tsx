...
import { findTrimCandidates, executeTrim } from '../utils/geometry/SketchTrimmer';
...
export const SketchPreview = () => {
    ...
    if (trimMode) {
      ...
      if (edge && edge.nodeIds.length >= 2) {
        ...
        if (startNode && endNode) {
          const candidates = findTrimCandidates(sketchNodes, sketchEdges, { 
          ...
          if (candidates.length > 0) {
            ...
            const result = executeTrim(sketchNodes, sketchEdges, candidate.edgeId, candidate.param);
            setSketchNodes(result.nodes);
            setSketchEdges(result.edges);
          }
        }
      }
      return;
  ...
  return (
    <group>
      ...
      {isSketchMode && activePlane && showConstraints && distanceConstraints.map((constraint) => {
      ...
      {isSketchMode && activePlane && showConstraints && angleConstraints.map((c) => {
      ...
      {isSketchMode && activePlane && showConstraints && geometricConstraints.map((constraint) => {
      ...
      {trimMode && (
        <Html position={[0, 0, 1]}>
          <div className="bg-primary/90 text-white px-4 py-2 rounded-lg shadow-lg flex items-center gap-2 text-[14px] font-bold animate-pulse">
            <span>✂️</span>
            <span>修剪模式已啟用 - 點擊要修剪的邊</span>
          </div>
        </Html>
      )}
    </group>
