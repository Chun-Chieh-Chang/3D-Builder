...
import { SimpleDatumPlanes } from './SimpleDatumPlanes';
...
const CameraHandler = () => {
  ...
  }, [activePlane, isSketchMode, cameraNormalFlip, controls]);
  ...
  useEffect(() => {
    ...
    if (!triggerNormal) return;

    const { activePlane: currentPlane, isSketchMode: currentSketchMode, cameraNormalFlip: currentFlip, controls: currentControls } = stateRef.current;

    if (!currentPlane) return;
    if (!currentSketchMode) return;

    const DISTANCE = 150;
    const dir = currentFlip ? -1 : 1;
    let targetPos = new THREE.Vector3(DISTANCE, DISTANCE, DISTANCE);
    let upVector = new THREE.Vector3(0, 1, 0);
    let targetOrbitCenter = new THREE.Vector3(0, 0, 0);

    if (currentPlane === 'FRONT') {
      targetPos.set(0, 0, DISTANCE * dir);
      upVector.set(0, 1, 0);
    } else if (currentPlane === 'TOP') {
      targetPos.set(0, DISTANCE * dir, 0);
      upVector.set(0, 0, dir === 1 ? -1 : 1);
    } else if (currentPlane === 'RIGHT') {
      targetPos.set(DISTANCE * dir, 0, 0);
      upVector.set(0, 1, 0);
    } else if (currentPlane === 'FACE') {
    ...
    } else if (currentPlane && currentPlane !== 'FACE') {
      const activeCustomPlane = useCadStore.getState().referencePlanes.find(p => p.id === currentPlane);
...
const OrbitControlsWrapper = React.memo(() => {
  const { isSketchMode, setControls, isCameraAnimating } = useCadStore();
...
export default function Viewport({ children }: ViewportProps) {
  const handlePointerMissed = useCallback(() => {
    console.log('[Selection] Clicked empty space. (No action for now)');
  }, []);
