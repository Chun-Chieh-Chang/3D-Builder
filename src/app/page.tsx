...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const unsubs = [
      ...
      onNewFile(() => {
        ...
        if (confirm('確定要建立新專案嗎？ All unsaved changes will be lost.')) {
  ...
  const { mode, setMode,
    ...
    hint, setHint,
    undo, redo, copy, cut, paste, selectAll, pushHistory,
    controls, setControls } = useCadStore();
  ...
  useEffect(() => {
    ...
    if (measurementMode === 'DISTANCE' && nextPts.length === 2) {
      ...
      if (edgeA && edgeB) {
        ...
        HeavyEngineClient.getInstance().measure(features).then(res => {
          setMeasurementResults({
            mode: 'AREA',
            value: res.area,
            unit: 'mm²',
            details: `Surface Area = ${res.area.toFixed(3)} mm²`
          });
        }).catch(err => {
        ...
        HeavyEngineClient.getInstance().measure(features).then(res => {
          setMeasurementResults({
            ...
            value: res.volume,
            unit: 'mm³',
            details: `Volume = ${res.volume.toFixed(3)} mm³`
          });
        }).catch(err => {
          console.error("Measurement failed:", err);
        });
  ...
  useEffect(() => {
    const checkHealth = async () => {
      ...
      setEngineStatus(isHealthy ? 'CONNECTED' : 'DISCONNECTED');
    };
  ...
  const [activeTab, setActiveTab] = useState<'FEATURES' | 'SKETCH' | 'EVALUATE' | 'ASSEMBLY' | 'DRAWING' | 'APPEARANCE'>('FEATURES');
  ...
  const handleNewProject = () => {
    if (confirm('確定要建立新專案嗎？所有未儲存的變更將會遺失。')) {
      useCadStore.setState({
        features: [],
        projectName: '新零件',
        meshData: [],
  ...
  return (
    ...
    <main className="flex flex-col h-screen w-screen overflow-hidden bg-[#EBEBEB] text-slate-800 font-sans">
      ...
      <header className="h-[32px] w-full bg-white border-b border-[#D1D5DB] flex items-center justify-between px-3 select-none z-30 shrink-0"> <div className="flex items-center gap-4"> <div className="flex items-center gap-1"> <span className="text-primary font-black text-[14px]">🔷</span> <span className="text-[14px] font-bold tracking-tight text-slate-800">SolidWeb 3D-Builder</span> </div> <nav className="flex items-center gap-3 text-[14px] text-slate-600 font-medium h-full">
            {/* File Menu */}
            <div className="relative h-full flex items-center">
              <span 
                className={`px-2 py-1 hover:bg-slate-100 hover:text-foreground cursor-pointer transition-all rounded ${activeMenu === 'FILE' ? 'bg-slate-100 text-foreground' : ''}`}
                onClick={(e) => { e.stopPropagation(); setActiveMenu(activeMenu === 'FILE' ? null : 'FILE'); }}
              >檔案(F)</span>
              {activeMenu === 'FILE' && (
                <div className="absolute top-[32px] left-0 bg-white border border-[#D1D5DB] shadow-xl z-[100] py-1 w-56 text-slate-800 font-normal animate-in fade-in slide-in-from-top-1 duration-100">
                  <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer flex justify-between items-center" onClick={() => { handleNewProject(); setActiveMenu(null); }}>
                    <span>新建...</span> <span className="text-xs text-slate-400">Ctrl+N</span>
                  </div>
                  <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer flex justify-between items-center" onClick={() => { handleOpen(); setActiveMenu(null); }}>
                    <span>開啟...</span> <span className="text-xs text-slate-400">Ctrl+O</span>
                  </div>
                  <div className="border-t border-slate-100 my-1"></div>
                  <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer flex justify-between items-center" onClick={() => { handleSave(); setActiveMenu(null); }}>
                    <span>儲存</span> <span className="text-xs text-slate-400">Ctrl+S</span>
                  </div>
                  <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer" onClick={() => { handleSave(); setActiveMenu(null); }}>另存為...</div>
                  <div className="border-t border-slate-100 my-1"></div>
                  <div className="group/export relative px-4 py-2 hover:bg-primary/10 cursor-pointer flex justify-between items-center">
                    <span>匯出</span> <span className="text-xs text-slate-400">▶</span>
                    <div className="absolute left-full top-0 ml-0.5 bg-white border border-[#D1D5DB] shadow-xl py-1 w-32 hidden group-hover/export:block">
                      <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer" onClick={() => { handleExportCad('STEP'); setActiveMenu(null); }}>STEP (.step)</div>
                      <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer" onClick={() => { handleExportCad('IGES'); setActiveMenu(null); }}>IGES (.iges)</div>
                      <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer" onClick={() => { handleExportCad('STL'); setActiveMenu(null); }}>STL (.stl)</div>
                    </div>
                  </div>
                  <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer" onClick={() => { handlePrintToPDF(); setActiveMenu(null); }}>列印為 PDF...</div>
                  <div className="border-t border-slate-100 my-1"></div>
                  <div className="px-4 py-2 hover:bg-red-50 text-red-600 cursor-pointer" onClick={() => { if(confirm('確定要結束程式嗎？')) window.close(); }}>結束(X)</div>
                </div>
              )}
            </div>
            ...
            <div className="relative h-full flex items-center">
              <span 
                className={`px-2 py-1 hover:bg-slate-100 hover:text-foreground cursor-pointer transition-all rounded ${activeMenu === 'EDIT' ? 'bg-slate-100 text-foreground' : ''}`}
                onClick={(e) => { e.stopPropagation(); setActiveMenu(activeMenu === 'EDIT' ? null : 'EDIT'); }}
              >編輯(E)</span>
              {activeMenu === 'EDIT' && (
                <div className="absolute top-[32px] left-0 bg-white border border-[#D1D5DB] shadow-xl z-[100] py-1 w-56 text-slate-800 font-normal animate-in fade-in slide-in-from-top-1 duration-100">
                  <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer flex justify-between items-center" onClick={() => { undo(); setActiveMenu(null); }}>
                    <span>復原 (Undo)</span> <span className="text-xs text-slate-400">Ctrl+Z</span>
                  </div>
                  <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer flex justify-between items-center" onClick={() => { redo(); setActiveMenu(null); }}>
                    <span>取消復原 (Redo)</span> <span className="text-xs text-slate-400">Ctrl+Y</span>
                  </div>
                  <div className="border-t border-slate-100 my-1"></div>
                  <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer flex justify-between items-center" onClick={() => { cut(); setActiveMenu(null); }}>
                    <span>剪下 (Cut)</span> <span className="text-xs text-slate-400">Ctrl+X</span>
                  </div>
                  <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer flex justify-between items-center" onClick={() => { copy(); setActiveMenu(null); }}>
                    <span>複製 (Copy)</span> <span className="text-xs text-slate-400">Ctrl+C</span>
                  </div>
                  <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer flex justify-between items-center" onClick={() => { paste(); setActiveMenu(null); }}>
                    <span>貼上 (Paste)</span> <span className="text-xs text-slate-400">Ctrl+V</span>
                  </div>
                  <div className="border-t border-slate-100 my-1"></div>
                  <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer flex justify-between items-center" onClick={() => { 
                    const state = useCadStore.getState();
                    ...
                    <span>刪除 (Delete)</span> <span className="text-xs text-slate-400">Del</span>
                  </div>
                  <div className="border-t border-slate-100 my-1"></div>
                  <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer flex justify-between items-center" onClick={() => { selectAll(); setActiveMenu(null); }}>
                    <span>選擇全部 (Select All)</span> <span className="text-xs text-slate-400">Ctrl+A</span>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="relative h-full flex items-center">
              <span 
                className={`px-2 py-1 hover:bg-slate-100 hover:text-foreground cursor-pointer transition-all rounded ${activeMenu === 'VIEW' ? 'bg-slate-100 text-foreground' : ''}`}
                onClick={(e) => { e.stopPropagation(); setActiveMenu(activeMenu === 'VIEW' ? null : 'VIEW'); }}
              >檢視(V)</span>
              {activeMenu === 'VIEW' && (
                <div className="absolute top-[32px] left-0 bg-white border border-[#D1D5DB] shadow-xl z-[100] py-1 w-56 text-slate-800 font-normal animate-in fade-in slide-in-from-top-1 duration-100">
                  <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer flex justify-between items-center">
                    <span>上一視圖</span> <span className="text-xs text-slate-400">Ctrl+Shift+Z</span>
                  </div>
                  <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer flex justify-between items-center">
                    <span>下一視圖</span> <span className="text-xs text-slate-400">Ctrl+Shift+Y</span>
                  </div>
                  <div className="border-t border-slate-100 my-1"></div>
                  <div className="group/view-orient relative px-4 py-2 hover:bg-primary/10 cursor-pointer flex justify-between items-center">
                    <span>視圖定向</span> <span className="text-xs text-slate-400">▶</span>
                    <div className="absolute left-full top-0 ml-0.5 bg-white border border-[#D1D5DB] shadow-xl py-1 w-40 hidden group-hover/view-orient:block">
                      <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer" onClick={() => {
                  ...
                  <div className="border-t border-slate-100 my-1"></div>
                  <div className="group/view-modify relative px-4 py-2 hover:bg-primary/10 cursor-pointer flex justify-between items-center">
                    <span>修改</span> <span className="text-xs text-slate-400">▶</span>
                    <div className="absolute left-full top-0 ml-0.5 bg-white border border-[#D1D5DB] shadow-xl py-1 w-40 hidden group-hover/view-modify:block">
                      <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer">動態旋轉 (Rotate)</div>
                      <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer">平移 (Pan)</div>
                      <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer">縮放 (Zoom)</div>
                    </div>
                  </div>
                  <div className="border-t border-slate-100 my-1"></div>
                  <div className="group/view-display relative px-4 py-2 hover:bg-primary/10 cursor-pointer flex justify-between items-center">
                    <span>顯示</span> <span className="text-xs text-slate-400">▶</span>
                    <div className="absolute left-full top-0 ml-0.5 bg-white border border-[#D1D5DB] shadow-xl py-1 w-48 hidden group-hover/view-display:block">
                      <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer">線架構 (Wireframe)</div>
                      <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer">消隱線 (Hidden Lines Removed)</div>
                      <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer">塗彩 (Shaded)</div>
                    </div>
                  </div>
                </div>
              )}
            ...
            {/* Insert Menu */}
            <div className="relative h-full flex items-center">
              <span className="px-2 py-1 hover:bg-slate-100 hover:text-foreground cursor-pointer transition-all rounded">插入(I)</span>
            </div>

            {/* Tools Menu */}
            <div className="relative h-full flex items-center">
              <span 
                className={`px-2 py-1 hover:bg-slate-100 hover:text-foreground cursor-pointer transition-all rounded ${activeMenu === 'TOOLS' ? 'bg-slate-100 text-foreground' : ''}`}
                onClick={(e) => { e.stopPropagation(); setActiveMenu(activeMenu === 'TOOLS' ? null : 'TOOLS'); }}
              >工具(T)</span>
              {activeMenu === 'TOOLS' && (
                <div className="absolute top-[32px] left-0 bg-white border border-[#D1D5DB] shadow-xl z-[100] py-1 w-56 text-slate-800 font-normal animate-in fade-in slide-in-from-top-1 duration-100">
                  <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer flex justify-between items-center" onClick={() => { handleRebuild(); setActiveMenu(null); }}>
                    <span>重建模型</span> <span className="text-xs text-slate-400">Ctrl+B</span>
                  </div>
                  <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer" onClick={() => { handleCalculateMassProperties(); setActiveMenu(null); }}>質量屬性...</div>
                  <div className="border-t border-slate-100 my-1"></div>
                  <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer" onClick={() => { alert('自訂介面將於未來版本開放'); setActiveMenu(null); }}>自訂(C)...</div>
                  <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer" onClick={() => { alert('系統選項將於未來版本開放'); setActiveMenu(null); }}>選項(O)...</div>
                </div>
              )}
            </div>
            ...
            <div className="relative h-full flex items-center">
              <span 
                className={`px-2 py-1 hover:bg-slate-100 hover:text-foreground cursor-pointer transition-all rounded ${activeMenu === 'INSERT' ? 'bg-slate-100 text-foreground' : ''}`}
                onClick={(e) => { e.stopPropagation(); setActiveMenu(activeMenu === 'INSERT' ? null : 'INSERT'); }}
              >插入(I)</span>
              {activeMenu === 'INSERT' && (
                <div className="absolute top-[32px] left-0 bg-white border border-[#D1D5DB] shadow-xl z-[100] py-1 w-56 text-slate-800 font-normal animate-in fade-in slide-in-from-top-1 duration-100">
                  <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer">凸台/基材 (Boss/Base)</div>
                  <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer">除料 (Cut)</div>
                  <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer">曲面 (Surface)</div>
                  <div className="border-t border-slate-100 my-1"></div>
                  <div className="group/insert-feat relative px-4 py-2 hover:bg-primary/10 cursor-pointer flex justify-between items-center">
                    <span>特徵 (Features)</span> <span className="text-xs text-slate-400">▶</span>
                    <div className="absolute left-full top-0 ml-0.5 bg-white border border-[#D1D5DB] shadow-xl py-1 w-40 hidden group-hover/insert-feat:block">
                      <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer">導角 (Chamfer)</div>
                      <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer">倒圓角 (Fillet)</div>
                      <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer">薄殼 (Shell)</div>
                    </div>
                  </div>
                  <div className="border-t border-slate-100 my-1"></div>
                  <div className="group/insert-ref relative px-4 py-2 hover:bg-primary/10 cursor-pointer flex justify-between items-center">
                    <span>參考幾何 (Reference Geometry)</span> <span className="text-xs text-slate-400">▶</span>
                    <div className="absolute left-full top-0 ml-0.5 bg-white border border-[#D1D5DB] shadow-xl py-1 w-48 hidden group-hover/insert-ref:block">
                      <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer">基準面 (Plane)</div>
                      <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer">基準軸 (Axis)</div>
                      <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer">基準點 (Point)</div>
                    </div>
                  </div>
                </div>
              )}
            ...
            <div className="relative h-full flex items-center">
              <span 
                className={`px-2 py-1 hover:bg-slate-100 hover:text-foreground cursor-pointer transition-all rounded ${activeMenu === 'HELP' ? 'bg-slate-100 text-foreground' : ''}`}
                onClick={(e) => { e.stopPropagation(); setActiveMenu(activeMenu === 'HELP' ? null : 'HELP'); }}
              >說明(H)</span>
              {activeMenu === 'HELP' && (
                <div className="absolute top-[32px] left-0 bg-white border border-[#D1D5DB] shadow-xl z-[100] py-1 w-56 text-slate-800 font-normal animate-in fade-in slide-in-from-top-1 duration-100">
                  <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer flex justify-between items-center">
                    <span>SolidWorks 說明</span> <span className="text-xs text-slate-400">F1</span>
                  </div>
                  <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer">搜尋 (Search)</div>
                  <div className="border-t border-slate-100 my-1"></div>
                  <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer">API 文件 (API Documentation)</div>
                  <div className="border-t border-slate-100 my-1"></div>
                  <div className="px-4 py-2 hover:bg-primary/10 cursor-pointer" onClick={() => { alert('SolidWeb 3D-Builder v1.0\\nPowered by OpenCASCADE & Three.js'); setActiveMenu(null); }}>關於 (About)</div>
                </div>
              )}
            </div>
          </nav> </div> <div className="text-[14px] text-slate-600 font-medium tracking-tight">

          零件1.SLDPRT * <span className="text-primary font-semibold">[{activePlane ? `${activePlane} ` : ''}]</span> </div> <div className="flex items-center gap-4 text-[14px] text-slate-600"> <div className="flex items-center gap-1.5"> <div className={`w-1.5 h-1.5 rounded-full ${engineStatus === 'CONNECTED' ? 'bg-success' : 'bg-error'}`} /> <span>OCCT 引擎狀態 : <span className={engineStatus === 'CONNECTED' ? 'text-success font-bold' : 'text-error'}>{engineStatus}</span></span> </div> <div className="flex gap-1"> <span className="w-8 h-8 flex items-center justify-center hover:bg-slate-100 cursor-pointer text-[14px]">─</span> <span className="w-8 h-8 flex items-center justify-center hover:bg-slate-100 cursor-pointer text-[14px]">□</span> <span className="w-8 h-8 flex items-center justify-center hover:bg-red-100 hover:text-red-600 cursor-pointer text-[14px]" onClick={() => { if(confirm('確定要結束程式嗎？')) window.close(); }}>✕</span> </div> </div> </header>
          ...
          {activeTab === 'FEATURES' ? (
            <div className="flex items-center gap-1.5 h-full overflow-x-auto no-scrollbar py-1">
              ...
              >
                ...
                <span className="text-[13px] text-slate-800 font-bold leading-none">伸長</span>
              </button>
              <button
                onClick={() => {
                  if (solidSketchPointCount >= 3) {
              ...
              >
                ...
                <span className="text-[13px] text-slate-800 font-bold leading-none">切除</span>
              </button>
              <button
                onClick={() => {
              ...
              >
                <span className="text-lg group-hover:scale-110 transition-all">🔄</span>
                <span className="text-[13px] text-slate-800 font-bold leading-none">旋轉</span>
              </button>
              ...
              >
                <span className="text-lg group-hover:scale-110 transition-all">🔢</span>
                <span className="text-[13px] text-slate-800 font-bold leading-none">陣列</span>
              </button>
            </div>
          ) : activeTab === 'SKETCH' ? (
            <div className="flex items-center gap-1.5 h-full overflow-x-auto no-scrollbar py-1">
              <button onClick={resetSketchSession} className="h-[60px] px-3 rounded bg-slate-100 hover:bg-slate-200 transition-all flex flex-col items-center justify-center gap-1 group border border-slate-300" title="✅ Exit Sketch (Apply Changes)">
                <span className="text-lg group-hover:scale-110 transition-all">✅</span>
                <span className="text-[13px] text-slate-800 font-bold leading-none">離開草圖</span>
              </button>
              <button onClick={() => setSmartDimensionActive(!smartDimensionActive)} className={`h-[60px] px-3 rounded transition-all flex flex-col items-center justify-center gap-1 group ${smartDimensionActive ? 'bg-primary/10 text-primary font-bold' : 'hover:bg-slate-200'}`} title="Dimension">
                <span className="text-lg group-hover:scale-110 transition-all"><svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="group-hover:scale-110 transition-all"><path d="M2 12h20M2 10v4M6 11v2M10 11v2M14 11v2M18 11v2M22 10v4"/></svg></span>
                <span className="text-[13px] leading-none">智慧尺寸</span>
              </button>
              <div className="w-[1px] h-[40px] bg-slate-300 mx-1 shrink-0" />
              <button onClick={() => setSketchTool('LINE')} className={`h-[60px] px-3 rounded transition-all flex flex-col items-center justify-center gap-1 group ${sketchTool === 'LINE' ? 'bg-primary/10 text-primary font-bold' : 'hover:bg-slate-200'}`} title="📏 Line (Continuous Segments)">
                <span className="text-lg group-hover:scale-110 transition-all"><svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="group-hover:scale-110 transition-all"><path d="M2 12h20M2 10v4M6 11v2M10 11v2M14 11v2M18 11v2M22 10v4"/></svg></span>
                <span className="text-[13px] leading-none">直線</span>
              </button>
              <button onClick={() => setSketchTool('CIRCLE')} className={`h-[60px] px-3 rounded transition-all flex flex-col items-center justify-center gap-1 group ${sketchTool === 'CIRCLE' ? 'bg-primary/10 text-primary font-bold' : 'hover:bg-slate-200'}`} title="⭕ Circle (Center-Radius)">
                <span className="text-lg group-hover:scale-110 transition-all">⭕</span>
                <span className="text-[13px] leading-none">圓</span>
              </button>
              <button onClick={() => setSketchTool('RECTANGLE')} className={`h-[60px] px-3 rounded transition-all flex flex-col items-center justify-center gap-1 group ${sketchTool === 'RECTANGLE' ? 'bg-primary/10 text-primary font-bold' : 'hover:bg-slate-200'}`} title="⬜ Rectangle (Corner-Corner)">
                <span className="text-lg group-hover:scale-110 transition-all">⬜</span>
                <span className="text-[13px] leading-none">矩形</span>
              </button>
            </div>
          ) : activeTab === 'DRAWING' ? (
            ...
            <div className="flex items-center gap-2 h-full">
              ...
              > <span className="text-lg group-hover:scale-110 transition-all"><svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="group-hover:scale-110 transition-all"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><path d="M9 15h3a1.5 1.5 0 0 0 0-3H9v6"/><path d="M14 12v6h1a2 2 0 0 0 2-2v-2a2 2 0 0 0-2-2h-1z"/></svg></span> <span className="text-[13px] leading-none font-bold">PDF Report</span> </button> <div className="w-[1px] h-[40px] bg-slate-300 mx-1 shrink-0" />
            ...
            <div className="flex items-center gap-1.5 h-full">
              ...
              <button
                ...
                title="測量 (Measure)">
                <span className="text-lg group-hover:scale-110 transition-all"><svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="group-hover:scale-110 transition-all"><path d="M2 12h20M2 10v4M6 11v2M10 11v2M14 11v2M18 11v2M22 10v4"/></svg></span> <span className="text-[13px] text-slate-800 font-bold leading-none">

                  {measurementMode !== 'NONE' ? '結束測量' : '測量'}
              ...
              {measurementMode !== 'NONE' && (
                ...
                <> <div className="w-[1px] h-[40px] bg-slate-300 mx-1 shrink-0" /> <button
                  ...
                  >
                    ...
                    <span className="text-sm"> </span> <span className="text-[13px] text-slate-700 font-bold">距離</span> </button> <button
                  ...
                  >
                    ...
                    <span className="text-sm"> </span> <span className="text-[13px] text-slate-700 font-bold">角度</span> </button> <button
                  ...
                  >
                    ...
                    <span className="text-sm"> </span> <span className="text-[13px] text-slate-700 font-bold">表面積</span> </button> <button
                  ...
                  >
                    ...
                    <span className="text-sm"> </span> <span className="text-[13px] text-slate-700 font-bold">體積</span> </button> </>
              ...
              > <span className="text-lg group-hover:scale-110 transition-all"> </span> <span className="text-[13px] font-bold leading-none text-slate-800">質量屬性</span> </button> <div className="w-[1px] h-[40px] bg-slate-300 mx-1 shrink-0" />
              ...
              >
                ...
                <span className="text-lg group-hover:scale-110 transition-all"> </span> <span className="text-[13px] font-bold leading-none text-slate-800">匯出 STEP</span> </button> <button
              ...
              >
                ...
                <span className="text-lg group-hover:scale-110 transition-all"> </span> <span className="text-[13px] font-bold leading-none text-slate-800">匯出 IGES</span> </button> <button
              ...
              >
                ...
                <span className="text-lg group-hover:scale-110 transition-all"> </span> <span className="text-[13px] font-bold leading-none text-slate-800">匯出 STL</span> </button> <button
              ...
              > <span className="text-lg group-hover:scale-110 transition-all"><svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="group-hover:scale-110 transition-all"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"/><polyline points="14 3 14 8 19 8"/><path d="M12 18l3-3-3-3M9 15h6"/></svg></span> <span className="text-[13px] font-bold leading-none text-slate-800">SLDPRT</span> </button> </div>
      ...
      <div className="flex-1 flex w-full overflow-hidden relative">
        ...
        <aside className="w-[290px] h-full bg-white border-r border-[#D1D5DB] flex flex-col z-10 shrink-0">
          ...
          <div className="h-[28px] w-full bg-[#F5F6F9] flex items-center justify-around border-b border-[#D1D5DB]/60 text-slate-500 text-[14px]"> <span className="text-primary font-bold cursor-pointer" title="FeatureManager 設計樹">特徵管理員</span> <span className="hover:text-slate-800 cursor-pointer" title="PropertyManager 屬性經理">屬性管理員</span> <span className="hover:text-slate-800 cursor-pointer" title="ConfigurationManager 設定經理">模型組態</span> </div> <div className="flex-grow flex flex-col overflow-hidden">

            {isSketchMode ? (

              /* Active Sketch Editor Panel */

              <div className="flex-grow overflow-y-auto p-3 bg-primary/5 border-l-4 border-primary"> <div className="text-[14px] uppercase tracking-wider text-primary mb-3 font-bold flex justify-between items-center"> <span>草圖編輯器</span> <button
          ...
          {!isSketchMode && selectedFeature && selectedSubNodeType !== 'SKETCH' && measurementMode === 'NONE' && (!selectedTopology || selectedTopology.type !== 'FACE') && (
            ...
            <div className="h-[250px] w-full border-t border-[#D1D5DB] bg-[#F5F6F9] flex flex-col p-3 z-10 shrink-0"> <div className="text-[14px] uppercase tracking-wider text-slate-500 mb-2 font-bold flex justify-between items-center border-b border-[#D1D5DB]/40 pb-1"> <span> PropertyManager</span> <span className="text-[13px] bg-primary/10 text-primary px-1 rounded uppercase font-mono">{selectedFeature.type}</span> </div> <div className="flex-1 overflow-y-auto space-y-2.5 pr-1">
                ...
                {selectedFeature.type === 'PATTERN' ? (
                  ...
                  <div className="bg-white p-2 rounded border border-[#D1D5DB] shadow-sm"> <div className="text-[13px] text-primary font-bold uppercase mb-1.5 border-b border-[#D1D5DB]/30 pb-0.5">陣列</div> <div className="space-y-2 text-[14px] pt-1">
                      ...
                      <div className="flex items-center justify-between gap-2"> <label className="text-[13px] text-slate-600 font-medium uppercase shrink-0">陣列</label> <select
                        ...
                        >
                          ...
                          <option value="CIRCULAR">陣列</option> <option value="LINEAR">陣列</option> </select> </div>
        ...
        <section className="flex-grow h-full relative" onContextMenu={(e) => e.preventDefault()}>
          ...
          {showMassPropsModal && massProps && (
            ...
            <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-[1000] p-4 animate-fade-in"> <div className="w-[520px] bg-slate-900/90 border border-slate-700/60 rounded-3xl p-6 shadow-2xl flex flex-col gap-5 text-slate-100 relative overflow-hidden backdrop-blur-xl">
                ...
                <div className="flex justify-between items-center border-b border-slate-800 pb-3 z-10"> <div className="flex items-center gap-2"> <span className="text-xl"> </span> <span className="text-[16px] font-extrabold tracking-wider uppercase text-amber-400 font-sans">質量屬性</span> </div> <button
