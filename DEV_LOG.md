...
---
## [2026-05-26] 視圖重置問題 (Viewport Reset Issue) - 根因分析與臨時修復 🚧

### 問題描述 (Problem Description)
- **症狀**：當縮放或旋轉視圖後，只要移動滑鼠或點擊平面，視圖就會自動重置到預設位置
- **影響**：嚴重影響用戶體驗，無法正常操作 3D 視圖

### Phase 1: Investigation (根因調查)
- **錯誤重現路徑**：
  1. 縮放視圖（滑鼠滾輪）
  2. 移動滑鼠 或 點擊基準面 或 按住滑鼠左鍵旋轉後放開
  3. 視圖立即重置到預設位置
- **觀察到的線索**：
  - 當游標在基準面上時放開滑鼠左鍵，問題特別容易重現
  - Console 中有狀態更新的日誌，顯示 store 正在被操作

### Phase 2: Pattern (模式分析)
- **正常範例**：當所有 3D 元件都被禁用（只留最基本的 PerspectiveCamera、OrbitControls、Grid）時，視圖不會重置
- **比對分析**：
  - 加入 Stage → 正常
  - 加入 DatumPlanes → 異常（問題開始出現）
  - 禁用 DatumPlanes → 正常
  - 確認：問題在 DatumPlanes 元件

### Phase 3: Hypothesis (假設分析 RCA)
- **根本原因 (Root Cause)**：
  1. **DatumPlanes 元件的滑鼠事件處理函數**：
     - `onPointerUp`、`onPointerDown`、`onPointerOver` 等事件會被觸發
     - 這些處理函數會操作 Zustand store（`setActivePlane`、`setSketchMode`、`setSelectedId` 等）
     - 操作 store 會導致元件重新渲染
     - 重新渲染會導致 OrbitControls 和 PerspectiveCamera 被重新創建或重置
...
- **CAPA 1 (臨時修復 - 已應用)**：
  ...
  - **完全禁用所有滑鼠事件**（不處理 onPointerOver、onPointerUp、onPointerDown 等）
...
- **CAPA 2 (完整修復 - 待執行)**：
  - 詳見 `handover_resume_guide.md` 中的「下一步修復計劃」
  - 核心策略：
    ...
    4. 將本地狀態（視覺相關）與 store 操作（功能相關）完全分離

### 關鍵文件變更
- 新增：`src/renderer/SimpleDatumPlanes.tsx` - 臨時簡化版本
- 修改：`src/renderer/Viewport.tsx` - 替換 DatumPlanes 為 SimpleDatumPlanes，簡化其他元件
- 新建：`handover_resume_guide.md` - 開發續寫指南
...
## [2026-05-25] Implementing Industrial Measurement & Mass Properties (OCCT Integration)

### Current State (Goal)
Move physical measurements (Area, Volume) from simplified client-side logic to the OCCT-powered backend for high accuracy.

### Implementation Details
- **Backend (geometry_service.py)**:
    - Integrated `OCC.Core.GProp` and `OCC.Core.BRepGProp`.
    - Implemented `calculate_properties(features)` which builds the raw B-Rep shape and computes surface area and volume using `brepgprop`.
- **Backend (geometry.py)**:
    - Added `@router.post("/measure")` endpoint.
- **Frontend (HeavyEngineClient.ts)**:
    - Added `measure(features)` method to call the new backend endpoint.
- **Frontend (page.tsx)**:
    - Updated `useEffect` to use `HeavyEngineClient.measure()` for AREA and VOLUME modes, replacing the hardcoded mock values.

### Verification
- [x] Backend compilation check: `python -m py_compile` passed.
- [x] Frontend type check: `npx tsc --noEmit` passed.
- [x] Verified that the integration follows the SolidWorks "Evaluate" roadmap.

### Next Steps
- Integrate a 2D geometric constraint solver for sketch mode.
- Add "Center of Mass" calculation to the measurement service.


---

## [2026-05-25] UI Optimization: Localized Navigation Labels

### Issue
The top navigation bar only displayed single letters (F, E, V, I, T, H), making it difficult for users to identify menu functions.

### Fix
- Updated `src/app/page.tsx` to include descriptive Chinese names for all top-level menus:
    - F -> 檔案(F) (File)
    - E -> 編輯(E) (Edit)
    - V -> 檢視(V) (View)
    - I -> 插入(I) (Insert)
...
### Verification
- Manually inspected JSX structure.
- Validated that the change aligns with standard CAD software UI conventions.
